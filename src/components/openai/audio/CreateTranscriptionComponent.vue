  ...
  <ion-content :fullscreen="true">
      <ion-grid>
        ...
        <ion-row>
          ...
          <select id="select-language" v-model="selectedLanguage">
            <option v-for="(languageCode, languageDescription) in languages" :key="languageCode" :value="languageCode">{{ languageDescription }}</option>
          </select>
        </ion-row>
...
name: 'CreateTranscriptionComponent',
...
setup(props, { emit }) {
  ...
  const selectedLanguage = ref(navigator.language.substring(0, 2))
  ...
  async function fetchOpenApi() {
    ...
    openAIApi.createAudioTranscription(audioBlob, fileName, selectedModel.value, prompt.value, temperature.value / 100, selectedLanguage.value).then(response => {
